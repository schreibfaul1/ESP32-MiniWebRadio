...
#define TFT_CONTROLLER          7                               // (0)ILI9341, (1)HX8347D, (2)ILI9486a, (3)ILI9486b, (4)ILI9488, (5)ST7796, (6)ST7796RPI, (7) RGB_HMI
...
private:
    struct FileInfo {
        ...
        char*    filePath;
            ...
            FileInfo(size_t g, const char* n, const char* p )
        : fileSize(g), fileName(strdup(n)), filePath(strdup(p)) {}
    };
...
private:
    ...
    void sort(){
        std::sort(m_files.begin(), m_files.end(), [](const FileInfo& a, const FileInfo& b) {
            ...
            return false; // strcmp(a.fileName, b.fileName) < 0;
        });
    }
};
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
