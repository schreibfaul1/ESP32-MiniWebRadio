...
boolean FtpServer::processCommand() {
    ...
    //    TRANSFER PARAMETER COMMANDS
        ...
        client.println("227 Entering Passive Mode (" + String(dataIp[0]) + "," + String(dataIp[1]) + "," + String(dataIp[2]) + "," + String(dataIp[3]) + "," + String(dataPort >> 8) + "," +
                       String(dataPort & 255) + ").");
        dataPassiveConn = true;
...
boolean FtpServer::dataConnect() {
    ...
    if (!data.connected()) { // wait 10 seconds for a data connection
        while (!dataServer.hasClient() && millis() - startTime < 10000) { yield(); }
...
void FtpServer::closeTransfer() {
    ...
    sprintf(chbuf, "Transfer in %lu.%03lus with %luKBytes/s ", (long unsigned int)(deltaT / 1000), (long unsigned int)(deltaT % 1000), (long unsigned int)(bytesTransfered / deltaT));
    if (ftp_debug) ftp_debug(chbuf);
    file.close();
    data.stop();
}
